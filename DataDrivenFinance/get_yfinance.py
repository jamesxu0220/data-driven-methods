...
def calculateRanks(sid: int):
    symbols = ['ABBV', 'ACN', 'AEP', 'AIZ', 'ALLE', 'AMAT', 'AMP', 'AMZN', 'AVB', 'AVY', 'AXP', 'BDX', 'BF-B', 'BMY', 'BR', 'CARR', 'CDW', 'CE', 'CHTR', 'CNC', 'CNP', 'COP', 'CTAS', 'CZR', 'DG', 'DPZ', 'DRE', 'DXC', 'META', 'FTV', 'GOOG', 'GPC', 'HIG', 'HST', 'JPM', 'KR', 'OGN', 'PG', 'PPL', 'PRU', 'PYPL', 'RE', 'ROL', 'ROST', 'UNH', 'URI', 'V', 'VRSK', 'WRK', 'XOM', 'IVV', 'IWM', 'EWU', 'EWG', 'EWL', 'EWQ', 'IEUS', 'EWJ', 'EWT',
               'MCHI', 'INDA', 'EWY', 'EWA', 'EWH', 'EWZ', 'EWC', 'IEMG', 'LQD', 'HYG', 'SHY', 'IEF', 'TLT', 'SEGA.L', 'IEAA.L', 'HIGH.L', 'JPEA.L', 'IAU', 'SLV', 'GSG', 'REET', 'ICLN', 'IXN', 'IGF', 'IUVL.L', 'IUMO.L', 'SPMV.L', 'IEVL.L', 'IEFM.L', 'MVEU.L', 'XLK', 'XLF', 'XLV', 'XLE', 'XLY', 'XLI', 'XLC', 'XLU', 'XLP', 'XLB', 'VXX', 'BTC-USD', 'ETH-USD', 'BNB-USD', 'ADA-USD', 'XRP-USD', 'SOL-USD', 'DOGE-USD', 'DOT-USD', 'SHIB-USD', 'AVAX-USD']
    ...
    dates_df = yf.download('AMZN', start=start_date,
                           end=end_date, progress=False).reset_index()[['Date']]
    dates = dates_df['Date']
    ...
    for s in symbols:
        ...
        df = df[df['Date'].isin(dates)][['Adj Close']].T
    ...
    for i, row in enumerate(prices):
        ActualPricesList.append(ActualPrices(submission_id=sid, symbol=symbols[i],
                                             price_day1=row[0], price_day2=row[1], price_day3=row[2],
                                             price_day4=row[3], price_day5=row[4], price_day6=row[5]))
    ...
    for i, row in enumerate(actual_ranks):
        ActualRanksList.append(ActualRanks(submission_id=sid, symbol=symbols[i],
